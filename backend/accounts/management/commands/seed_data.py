...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        exec1, _ = User.objects.get_or_create(
            username='rahul',
            defaults={
        ...
        exec1.set_password('exec123')
        exec1.save()

        exec2, _ = User.objects.get_or_create(
            username='anita',
            defaults={
        ...
        # Create clients
        clients_data = [
            {'company_name': 'AgriGlobal LLC', 'country': 'USA', 'city': 'Houston', 'business_type': 'Distributor', 'preferred_currency': 'USD', 'credit_days': 30, 'credit_limit': 50000, 'delivery_terms': 'CIF', 'primary_executive': exec1},
            {'company_name': 'EuroFarm GmbH', 'country': 'Germany', 'city': 'Hamburg', 'business_type': 'Importer', 'preferred_currency': 'EUR', 'credit_days': 45, 'credit_limit': 75000, 'delivery_terms': 'FOB', 'primary_executive': exec1},
            {'company_name': 'AgroTech Kenya', 'country': 'Kenya', 'city': 'Nairobi', 'business_type': 'Distributor', 'preferred_currency': 'USD', 'credit_days': 30, 'credit_limit': 25000, 'delivery_terms': 'CFR', 'primary_executive': exec2},
            {'company_name': 'BioHarvest Brazil', 'country': 'Brazil', 'city': 'Sao Paulo', 'business_type': 'Manufacturer', 'preferred_currency': 'USD', 'credit_days': 60, 'credit_limit': 100000, 'delivery_terms': 'FOB', 'primary_executive': exec2},
            {'company_name': 'GreenFields Vietnam', 'country': 'Vietnam', 'city': 'Ho Chi Minh', 'business_type': 'Importer', 'preferred_currency': 'USD', 'credit_days': 30, 'credit_limit': 30000, 'delivery_terms': 'CIF', 'primary_executive': exec1},
        ]
        clients = []
        ...
        # Create contacts for first two clients
        ...
        Inquiry.objects.get_or_create(
            ...
            defaults={'assigned_to': exec1, 'source': 'email', 'stage': 'quotation', 'quantity': '10 MT', 'expected_value': 12000, 'requirements': 'Need 25kg bags, palletized'}
        )
        Inquiry.objects.get_or_create(
            client=clients[1], product_name='Seaweed Extract',
            defaults={'assigned_to': exec1, 'source': 'whatsapp', 'stage': 'discussion', 'quantity': '5 MT', 'expected_value': 12500}
        )
        Inquiry.objects.get_or_create(
            client=clients[2], product_name='NPK 19-19-19',
            defaults={'assigned_to': exec2, 'source': 'manual', 'stage': 'inquiry', 'quantity': '20 MT', 'expected_value': 16000}
        )

        # Create tasks
        Task.objects.get_or_create(
            title='Follow up on Humic Acid inquiry - AgriGlobal',
            defaults={'client': clients[0], 'owner': exec1, 'created_by': admin, 'priority': 'high', 'description': 'Client interested in 10 MT. Prepare quotation.'}
        )
        Task.objects.get_or_create(
            title='Send product catalog to EuroFarm',
            defaults={'client': clients[1], 'owner': exec1, 'created_by': manager, 'priority': 'medium'}
        )
        Task.objects.get_or_create(
            title='Collect feedback on NPK sample from AgroTech',
            defaults={'client': clients[2], 'owner': exec2, 'created_by': admin, 'priority': 'high', 'is_auto_generated': True}
        )
        ...
        self.stdout.write('  Manager:   priya / manager123')
        self.stdout.write('  Executive: rahul / exec123')
        self.stdout.write('  Executive: anita / exec123')
