...
export default function CommunicationsPage() {
  ...
  const filteredList = useMemo(() => {
    let filtered = list;
    if (filterTab === "unmatched") filtered = filtered.filter((item) => !item.client);
    else if (filterTab !== "all") filtered = filtered.filter((item) => item.comm_type === filterTab);
    ...
    return filtered;
  }, [list, filterTab, filterClient, filterExec]);
  ...
  return (
    <div>
      ...
      {/* Accept Client Modal */}
      <Modal open={!!assignModal} onClose={() => { setAssignModal(null); setShowAddClient(false); setNewClientName(""); }} title="Accept Client">
        <div className="space-y-4">
          <p className="text-sm text-gray-600">This communication was not auto-matched to any client. Select an existing client or add a new one:</p>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Client *</label>
            ...
            {showAddClient ? (
              <button onClick={async () => {
                ...
                try {
                  ...
                  await api.patch(`/communications/${assignModal}/`, { client: res.data.id });
                  toast.success(`Client "${newClientName}" created and assigned`);
                  setAssignModal(null); setShowAddClient(false); setNewClientName("");
                  setClients(prev => [...prev, res.data]);
                  dispatch(fetchCommunications());
                } catch (err) { toast.error(getErrorMessage(err, "Failed")); }
              }} disabled={!newClientName.trim()} className="px-6 py-2 bg-teal-600 text-white rounded-lg font-medium hover:bg-teal-700 disabled:opacity-50">Add & Accept</button>
            ) : (
              <button onClick={handleAssignClient} disabled={!assignClient} className="px-6 py-2 bg-indigo-600 text-white rounded-lg font-medium hover:bg-indigo-700 disabled:opacity-50">Accept</button>
            )}
